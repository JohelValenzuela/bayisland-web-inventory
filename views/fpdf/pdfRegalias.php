...
class PDF extends FPDF {
    ...
    function Header()
   {
    ...
    $this->Image('build/img/logo.png', 3, 3, 50); // Y  W  H
      ...
      $this->Cell(220, 15,  iconv('UTF-8', 'windows-1252', ('REPORTE DE REGALÍA DE PRODUCTOS')), 0, 1, 'C', 0); // AnchoCelda,AltoCelda,titulo,borde(1-0),saltoLinea(1-0),posicion(L-C-R),ColorFondo(1-0)
...
foreach ($regalia as $regalias) {
    $regalias->producto = Producto::find($regalias->producto_id);
    $regalias->usuario = Auth::find($regalias->usuario_id);
    $regalias->medida = UnidadMedida::find($regalias->producto->medidaId);
...
foreach($fila = $regalia as $regalia){
   $pdf->Cell(10, 8, iconv('UTF-8', 'windows-1252', $regalia->id), 1, 0, 'C', 0);
   $pdf->Cell(50, 8, iconv('UTF-8', 'windows-1252', $regalia->usuario->nombre . " " . $regalia->usuario->apellido), 1, 0, 'C', 0);  
   $pdf->Cell(60, 8, iconv('UTF-8', 'windows-1252', $regalia->producto->nombre . " " . $regalia->producto->cantidadPresentacion . " " . $regalia->medida->sigla), 1, 0, 'C', 0);  
   $pdf->Cell(30, 8, iconv('UTF-8', 'windows-1252', $regalia->cantidad), 1, 0, 'C', 0); 
   $pdf->Cell(40, 8, iconv('UTF-8', 'windows-1252', $regalia->observacion), 1, 0, 'C', 0); 
   $pdf->Cell(50, 8, iconv('UTF-8', 'windows-1252', $regalia->fecha_regalia), 1, 0, 'C', 0); 
   $pdf->Cell(35, 8, iconv('UTF-8', 'windows-1252', $regalia->estado), 1, 1, 'C', 0);
